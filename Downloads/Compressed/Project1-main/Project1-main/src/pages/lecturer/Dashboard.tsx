nges  import * as React from 'react';
...
type StudentStatus = 'Normal' | 'Excelling' | 'Behind' | 'Flagged';
...
const DEFAULT_STUDENTS: Student[] = [
  { id: 's1', name: 'Alex Johnson', company: 'Google', progress: 45, status: 'Normal', avatar: 'A' },
  { id: 's2', name: 'Chloe Davies', company: 'Atlassian', progress: 82, status: 'Excelling', avatar: 'C' },
  { id: 's3', name: 'Daniel Smith', company: 'Canva', progress: 12, status: 'Behind', avatar: 'D' },
  { id: 's4', name: 'Elena Rodriguez', company: 'Stripe', progress: 60, status: 'Normal', avatar: 'E' },
  { id: 's5', name: 'Felix Brown', company: 'Airbnb', progress: 30, status: 'Flagged', avatar: 'F' },
];

export default function LecturerDashboard() {
  const currentUser = MOCK_USERS[2]; // Dr. Sarah Smith
  const [students, setStudents] = React.useState<Student[]>(DEFAULT_STUDENTS);
  const [addOpen, setAddOpen] = React.useState(false);

  const [form, setForm] = React.useState<{ name: string; company: string; progress: string; status: StudentStatus; avatar: string }>({
    name: '',
    company: '',
    progress: '0',
    status: 'Normal',
  ...
  const resetForm = React.useCallback(() => {
    setForm({ name: '', company: '', progress: '0', status: 'Normal', avatar: '' });
    setFormError('');
  }, []);

  const handleSubmitAddStudent = React.useCallback(() => {
    setFormError('');

    const name = form.name.trim();
    const company = form.company.trim();
    const progressNum = Number(form.progress);
    const avatarLetter = (form.avatar.trim() ? form.avatar.trim()[0] : name[0] || 'S').toUpperCase();
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-8 border-b border-slate-100">
        <div className="space-y-4">
          ...
          <Button
            ...
            className="bg-primary-600 hover:bg-primary-700 text-white rounded-2xl h-16 px-10 font-black uppercase tracking-widest text-[10px] shadow-xl shadow-primary-600/10 transition-all hover:scale-105 border-none"
          >
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-[720px]">
          <DialogHeader>
            ...
            <DialogDescription>Lecturer can add students to their supervision list.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="student-company">Company / Placement</Label>
              <Input
                id="student-company"
                value={form.company}
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500/30"
              >
            ...
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="student-avatar">Avatar Letter (optional)</Label>
              <Input
                id="student-avatar"
                value={form.avatar}
              ...
              />
              <Textarea
                value={formError ? `Error: ${formError}` : ''}
                readOnly
                className="hidden"
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitAddStudent} className="bg-primary-600 hover:bg-primary-700">
              Add Student
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <Card className="lg:col-span-2 glass border-slate-200/50 overflow-hidden shadow-xl">
          <CardHeader className="p-8 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6 bg-slate-50/20">
             <div className="flex items-center gap-4">
               ...
               <Badge className="bg-slate-100 text-slate-500 border-none font-bold text-[9px] uppercase tracking-widest">{assignedCount} Total</Badge>
             </div>
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-slate-100">
              {students.map((student) => (
                <div key={student.id} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors group cursor-pointer">
                  ...
                  <div className="flex flex-col items-end gap-3">
                    <Badge
                      className={`text-[9px] font-bold uppercase tracking-widest border-none px-3 py-1.5 shadow-sm ${
