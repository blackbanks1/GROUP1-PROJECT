...
function StatusSummary({ label, count, color, textColor = 'text-slate-900' }: { label: string; count: string; color: string; textColor?: string }) {
  return (
...
function ApplicationCard({ app, index }: { app: any; index: number; key?: React.Key }) {
  const statusColors: any = {
    'Interview': 'bg-primary-50 text-primary-600 border-primary-200',
    'In Review': 'bg-slate-50 text-slate-600 border-slate-200',
    'Accepted': 'bg-primary-600 text-white border-primary-700',
  ...
  return (
    ...
    >
      <Card className="bg-white border-none hover:shadow-xl hover:shadow-primary-600/5 transition-all group cursor-pointer overflow-hidden p-10 rounded-[2.5rem] shadow-sm relative">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-10 relative z-10">
          <div className="flex items-center gap-8 flex-1">
             <div className="w-24 h-24 rounded-[2rem] bg-slate-50 border border-slate-100 p-4 flex items-center justify-center group-hover:bg-white group-hover:border-primary-100 transition-all shadow-sm">
                <img src={app.logo} alt={app.company} className="w-full h-full rounded-2xl object-cover" />
             </div>
             <div className="space-y-2">
                <h4 className="text-3xl font-bold tracking-tight text-slate-950 group-hover:text-primary-600 transition-colors font-display italic uppercase leading-none">{app.role}</h4>
                <div className="flex items-center gap-6 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                   <span className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100"><Building2 className="w-3.5 h-3.5 text-primary-600" /> {app.company}</span>
                   <span className="flex items-center gap-2"><MapPin className="w-3.5 h-3.5" /> {app.location}</span>
                </div>
             </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-10 lg:justify-end">
             <div className="text-right hidden lg:block">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 mb-2 font-display italic">Timeline</p>
                <p className="text-base font-bold text-slate-600 italic font-display">{app.appliedDate}</p>
             </div>
             <div className="w-px h-12 bg-slate-100 hidden lg:block" />
             <Badge className={`h-14 px-10 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] border flex items-center gap-3 shadow-sm transition-transform active:scale-95 ${statusColors[app.status]}`}>
               {StatusIcon[app.status]}
               {app.status}
             </Badge>
             <Button variant="outline" size="icon" className="w-14 h-14 rounded-2xl bg-white text-slate-300 group-hover:text-white group-hover:bg-primary-600 group-hover:border-primary-600 border-slate-200 transition-all shadow-sm hover:scale-110">
